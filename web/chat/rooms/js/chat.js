...
var ajaxChat = {
	...
	DOMbufferRowClass: 'rowOdd',
        websocketServerEnabled: null,
        websocketServerHost: null,
	...
	mergeArrays: function(array1, array2, array3) {
		new_array = new Array();
		for(var i1=0; i1<array1.length; i1++) {
			new_array[new_array.length] = this.encodeSpecialChars(array1[i1]);
		}
		for(var i2=0; i2<array2.length; i2++) {
			new_array[new_array.length] = this.encodeSpecialChars(array2[i2]);
		}
		for(var i3=0; i3<array3.length; i3++) {
			new_array[new_array.length] = this.encodeSpecialChars(array3[i3]);
		}
		return new_array;
	},
	...
	initConfig: function(config) {
		...
		this.emoticonCodes1			= config['emoticonCodes'];
		this.emoticonFiles1			= config['emoticonFiles'];
		this.emoticonCodes2			= config['emoticonCodes2'];
		this.emoticonFiles2			= config['emoticonFiles2'];
		this.emoticonCodes			= this.mergeArrays(config['emoticonCodes'], config['emoticonCodes2'], config['emoticonCodesHidden']);
		this.emoticonFiles			= this.mergeArrays(config['emoticonFiles'], config['emoticonFiles2'], config['emoticonFilesHidden']);
		...
		this.websocketServerEnabled	= config['websocketServerEnabled'];
		this.websocketServerHost	= config['websocketServerHost'];
		this.websocketServerPort	= config['websocketServerPort'];
	...
	initEmoticons: function() {
		this.DOMbuffer = "";
		for(var i=0; i<this.emoticonCodes1.length; i++) {
			// Replace specials characters in emoticon codes:
			this.emoticonCodes1[i] = this.encodeSpecialChars(this.emoticonCodes1[i]);
			this.DOMbuffer = this.DOMbuffer
						+ '<a href="javascript:ajaxChat.insertText(\''
						+ this.scriptLinkEncode(this.emoticonCodes1[i])
						+ '\');ajaxChat.showHide(\'emoticonsContainer\', null);"><img src="'
						+ this.dirs['emoticons']
						+ this.emoticonFiles1[i]
						+ '" alt="'
						+ this.emoticonCodes1[i]
						+ '" title="'
						+ this.emoticonCodes1[i]
						+ '"/></a>';
			}
		if(this.dom['emoticons']) {
 			this.updateDOM('emoticons', this.DOMbuffer);
 		}
		this.DOMbuffer = "";
		for(var i=0; i<this.emoticonCodes2.length; i++) {
			// Replace specials characters in emoticon codes:
			this.emoticonCodes2[i] = this.encodeSpecialChars(this.emoticonCodes2[i]);
			this.DOMbuffer = this.DOMbuffer
						+ '<a href="javascript:ajaxChat.insertText(\''
						+ this.scriptLinkEncode(this.emoticonCodes2[i])
						+ '\');ajaxChat.showHide(\'emoticons2Container\', null);"><img src="'
						+ this.dirs['emoticons']
						+ this.emoticonFiles2[i]
						+ '" alt="'
						+ this.emoticonCodes2[i]
						+ '" title="'
						+ this.emoticonCodes2[i]
						+ '"/></a>';
			}
		if(this.dom['emoticons2']) {
 			this.updateDOM('emoticons2', this.DOMbuffer);
 		}
 		this.DOMbuffer = "";
	},
	
	initColorCodes: function() {
		if(this.dom['colorCodes']) {
			this.DOMbuffer = "";
			...
			this.updateDOM('colorCodes', this.DOMbuffer);
 			this.DOMbuffer = "";
		}
	},
	...
	websocketConnect: function() {
		...
		if(this.websocket === null && this.websocketServerEnabled) {
			try {
                            if ('WebSocket' in window) {
                                var host = 'wss://' + this.websocketServerHost + ':' + this.websocketServerPort + '/worker';
                                console.log('Connect');
                                console.log(host);
                                ...
                                this.websocket.onopen = function() {
                                    var socketRequest = {
                                        'contextType': 110,
                                        'contextId': ajaxChat.channelID
                                    };
                                    ajaxChat.websocket.send(JSON.stringify(socketRequest));

                                    var socketRequest = {
                                        'contextType': 110,
                                        'contextId': parseInt(ajaxChat.userID) + ajaxChat.privateMessageDiff
                                    };
                                    ajaxChat.websocket.send(JSON.stringify(socketRequest));
                                };
                                ...
                                this.websocket.onclose = function() {
                                    ajaxChat.websocket = null;
                                    if (ajaxChat.websocketManualClose == false) {
                                        ajaxChat.updateChat(null);
                                    }
                                };
                            }
        ...
        websocketChannelChange: function(oldChannel, newChannel) {
            if (this.websocket !== null) {                
                var socketRequest = {
                    'action': 'remove',
                    'contextType': 110,
                    'contextId': oldChannel
                };
                ajaxChat.websocket.send(JSON.stringify(socketRequest));
                var socketRequest = {
                    'contextType': 110,
                    'contextId': newChannel
                };
                ajaxChat.websocket.send(JSON.stringify(socketRequest));
            }
        },
	...
	handleXML: function(xmlDoc) {
		...
		this.handleChannels(xmlDoc.getElementsByTagName('channel'));
	...
	setChatUpdateTimer: function() {
		...
		if(this.chatStarted) {
			var timeout;
			if(this.websocket !== null) {
                                // :TODO: JOE - check if this is actually working
				timeout = (this.inactiveTimeout * 60000) - 10000;
			} else {
	...
	handleInfoMessage: function(infoType, infoData) {
		switch(infoType) {
			...
			case 'channelName':
				...
				break;
			case 'channelID':
                                this.websocketChannelChange(this.channelID, infoData);
	...
	handleOnlineUsers: function(userNodes) {
		if(userNodes.length) {
			...
			for(var i=0; i<userNodes.length; i++) {
				...
				chatMarker = userNodes[i].getAttribute('chatMarker');
	...
	handleChannels: function(channelNodes) {
		var roomList = document.getElementById('roomList');
		var changes = false;
		var existingRooms = new Array();
		// Update existing channels and add new channels if some exists
		for(var i=0; i<channelNodes.length; i++) {
		  /*
			roomListHtml = roomListHtml + '<div id="channel' + channelId # '" class="row>';
			
			
			roomListHtml = roomListHtml + '</div>';
		  
				var newChannel = document.createElement('div');
				newChannel.setAttribute('id', 'channel' + channelId);
				newChannel.setAttribute('class', '');
				newChannel.innerHTML = '<a id="channelLink' + channelId + '" class="inactivechannel" href="#" onclick="ajaxChat.switchChannel(\'' + channelNodes[i].firstChild.nodeValue + '\', ' + channelId + ');">' +
				                       '<span id="channelName' + channelId + '">' + channelNodes[i].firstChild.nodeValue + '</span> ' +
				                       '<span id="channelUserCount' + channelId + '">(' + channelNodes[i].getAttribute('user_count') + ')</span>' +
				                       '</a>';
							
				var roomList = document.getElementById('roomList');
				roomList.appendChild(newChannel);
				
				
		roomList.innerHTML = roomListHtml;
		*/
		  
			channelId = channelNodes[i].getAttribute('id');
			var elementId = 'channel' + channelId;
			existingRooms.push(elementId);
			var channel= document.getElementById(elementId); 
			if (channel != null) {
				var channelLink = document.getElementById('channelLink' + channelId);
				if(channelLink != null) {
					if(this.channelID == channelId) {
						channelLink.className = 'activechannel';
					} else {
						channelLink.className = 'inactivechannel';
					}
					if(channelNodes[i].getAttribute('invite_only') == 1) {
                                            channelLink.className += ' inviteonly';
                                        }
				}
				var channelName = document.getElementById('channelName' + channelId);
				if(channelName != null) {
					channelName.innerHTML = channelNodes[i].firstChild.nodeValue;
				}
				var channelUserCount = document.getElementById('channelUserCount' + channelId);
				if(channelUserCount != null) {
					channelUserCount.innerHTML = "(" + channelNodes[i].getAttribute('user_count') + ")";
			}
			} else {
				var newChannel = document.createElement('div');
				newChannel.setAttribute('id', 'channel' + channelId);
		...
		// Removes closed rooms from the list
		for (var i=0; i<roomList.childNodes.length; i++) {
		// alert('Check [' + roomList.childNodes[i].id + ']');
		  
			if(roomList.childNodes[i].id != null && !this.in_array(roomList.childNodes[i].id, existingRooms)) {
				roomList.removeChild(document.getElementById(roomList.childNodes[i].id));
				changes = true;
			}
		}
		// Update the rowEven/rowOdd-classes
		if (changes) {
			for (var i=0; i<roomList.childNodes.length; i++) {
			// alert("[" + roomList.childNodes[i].getAttribute('id') + "  -   " + (i % 2) + "]");
				if(roomList.childNodes[i].id != null) {
					document.getElementById(roomList.childNodes[i].id).className = (i % 2 == 0 ? 'rowEven' : 'rowOdd');
				}
			}
		}
		/*
		...
		if(this.in_array(roomList.childNodes[i].getAttribute('id'), existingRooms) {
		  
				roomList.removeChild(roomList.childNodes[i]);
			}
		}
		*/
	},
	...
	addUserToOnlineList: function(userID, userName, userRole, chatMarker) {
		this.usersList.push(userID);
		this.userNamesList.push(userName);	
		if(this.dom['onlineList']) {
			this.updateDOM(
				'onlineList',
				this.getUserNodeString(userID, userName, userRole, chatMarker),
				(this.userID == userID)
			);
		}
	},

	getUserNodeString: function(userID, userName, userRole, chatMarker) {
		if(this.userNodeString && userID == this.userID) {
			...
			var str	= '<div id="'
					...
					+ '"><a href="https://www.veggiecommunity.org/' + vcLang 
					+ '/profile/' + userID + '/" target="_blank" class="vcprofile">'
					+ '<img src="img/thumbnail.png" width="16" height="16" /></a> '
					
					+ '<a href="javascript:ajaxChat.insertMessageWrapper(\'/msg '
					+ encodedUserName
					+ ' \')" class="vcprofile">'
					+ '<img src="img/msg.png" width="16" height="16" /></a> '
	...
	switchChannel: function(channel, channel_id) {
		if(!this.chatStarted) {
			this.clearChatList();
			this.channelSwitch = true;
		...
		clearTimeout(this.timer);	
		var message = 'lastID=' + this.lastID + '&channelID=' + channel_id;
		this.makeRequest(this.ajaxURL,'POST',message);
		if(this.dom['inputField'] && this.settings['autoFocus']) {
			this.dom['inputField'].focus();
		}
	...
	replaceBBCodeImage: function(url) {
		if(this.settings['bbCodeImages']) {
			...
			var maxWidth = 400; // this.dom['chatList'].offsetWidth-50;
			var maxHeight = 110; // this.dom['chatList'].offsetHeight-50;
			return	'<a href="'
					+url
					+'" onclick="window.open(this.href); return false;">'
	...
	finalize: function() {	
		if(typeof this.finalizeFunction == 'function') {
			this.finalizeFunction();
                ...
                if (this.websocket !== null) {
                    this.websocketManualClose = true;
