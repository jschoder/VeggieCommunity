...
class AJAXChat {
	...
	function login() {
		...
		if (!empty($userData['createdAt']) && $userData['createdAt'] + 86400 > time()) {
			$this->addInfoMessage('errorNewUserBlock');
			return false;
		}
		...
		$this->setChatMarker($userData['chatMarker']);
	...
	function addToOnlineList() {
		$sql = 'INSERT INTO '.$this->getDataBaseTable('online').'(
				...
				VALUES (
					...
					'.$this->db->makeSafe($this->getChatMarker()).',
	...
	function insertParsedMessage($text) {
		...
		if(strpos($text, '/') === 0) {
			...
			switch($textParts[0]) {
				...
				case '/create':
					$this->insertParsedMessageCreate($textParts, 0);
					break;
					
				// Create a new private channel
				case '/private':
					$this->insertParsedMessageCreate($textParts, 1);
					break;
                                    /*
...
*/ 
	...
	function insertParsedMessageCreate($textParts, $inviteOnly = 0) {
		if(count($textParts) == 1) {
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error MissingChannelName'
			...
			$channel_name = implode('_', $textParts);
			$channel_name = str_ireplace(array('\'', '"'), array('', ''), $channel_name);
			$channel_name = $this->convertEncoding($channel_name, $this->getConfig('contentEncoding'), $this->getConfig('sourceEncoding'));
			$channel_name = mysqli_real_escape_string($this->db->getConnectionID(), $channel_name);

			$query = sprintf("SELECT channel_id FROM ajax_chat_channels WHERE channel_name='%s'",
			                 $channel_name);
			...
			// Only create the new room if it doesn't exist yet
			if($result->numRows() === 0) {
				$query = sprintf("INSERT INTO ajax_chat_channels (channel_id,channel_name,manual_channel, created,created_by, invite_only) " .
				                 "SELECT max(channel_id) + 1, '%s', '1', NOW(), " . intval($this->getUserID()) . ", " . intval($inviteOnly) . " FROM ajax_chat_channels",
				                 $channel_name);
				$result = $this->db->sqlQuery($query);
				...
				// Reset the local channel-cache
				$this->getAllChannels(true);
				$this->getChannels(true); 
			}
			// Autojoin the new channel
			$this->switchChannel($channel_name);
		}
	}
	...
	function deleteMessage($messageID) {
		...
		if($row['channel'] !== null) {
			...
			// Backup deleted message for later review
			$sql = 'INSERT INTO ajax_chat_messages_censored ' .
			       'SELECT ' . $this->getDataBaseTable('messages') . '.*, NOW(), ' . $this->getUserID() . 
			       ' FROM ' . $this->getDataBaseTable('messages') .
			       ' WHERE id='.$this->db->makeSafe($messageID).' ' . 
			       $condition. ';';
	...
	function getChatViewOnlineUsersXML($channelIDs) {
		...
		foreach($onlineUsersData as $onlineUserData) {
			$xml .= '<user' . 
			        ...
			        ' userRole="'.$onlineUserData['userRole'].'"' . 
			        ' chatMarker="'.$onlineUserData['chatMarker'].'"' . 
			        ' channelID="'.$onlineUserData['channel'].'"' . 
                                '>' . 
			        '<![CDATA['.$this->encodeSpecialChars($onlineUserData['userName']).']]>' . 
			        '</user>';
		}
		$xml .= '</users>';	
		return $xml;
	...
	function getChatViewMessageXML(
		...
		return '<message id="'.$messageID.'" dateTime="'.date('r', $timeStamp).'" userID="'.$userID.'" userRole="'.$userRole.'" channelID="'.$channelID.'">' . 
                          '<username><![CDATA['.$this->encodeSpecialChars($userName).']]></username>' . 
                          '<text><![CDATA['.$this->encodeSpecialChars($text).']]></text>' . 
                       '</message>';
	}
	...
	function getChatViewMessagesXML() {
		...
		if($this->getConfig('requestMessagesPriorChannelEnter') ||
		   ($this->getConfig('requestMessagesPriorChannelEnterList') && 
		    in_array($this->getChannel(), $this->getConfig('requestMessagesPriorChannelEnterList')))) {
			$firstMessageTimestamp = time()  - ($this->getConfig('requestMessagesTimeDiff') * 3600);
		} else {
			$firstMessageTimestamp = $this->getChannelEnterTimeStamp();
		}
		$query = 'SELECT
                                id,
                                userID,
		...
		$queryParams = array(
			intval($this->getChannel()),
			intval($this->getPrivateMessageID()),
			intval($this->getRequestVar('lastID')),
			date('Y-m-d H:i:00', $firstMessageTimestamp)
		);
				
                $statement = $this->db->sqlPreparedQuery($query, $queryParams);
                
                // Stop if an error occurs:
                if(empty($statement)) {
                        echo 'ChatMessages-Statement failed';
                        die();
                }
	...
	function getOnlineUsersData($channelIDs=null, $key=null, $value=null) {
		if($this->_onlineUsersData === null) {
			...
			$sql = 'SELECT
                                ...
                                ORDER BY
                                        userName';
                        $statement = $this->db->sqlPreparedQuery($sql, null);
                        
                        // Stop if an error occurs:
                        if(empty($statement)) {
                                echo 'OnlineUser-Statement failed';
                                die();
                        }
                        $binded = $statement->bind_result(
                            $userID,
                        ...
                        while ($statement->fetch()) {
                            $this->_onlineUsersData[] = array(
                                ...
                                'channel' => $channel,
                                'timeStamp' => $timeStamp,
                                'ip' => $this->ipFromStorageFormat($ip),
                            );
                        }
                        $statement->close();
		}
	...
	function setChatMarker($chatMarker) {
		$this->setSessionVar('ChatMarker', $chatMarker);
	}

	function getChatMarker() {		
		$chatMarker = $this->getSessionVar('ChatMarker');
		if($chatMarker === null)
			return '';
		return $chatMarker;
	}
	...
	function clearManualChannels() {
		// Turn unused channels into "Zombies" (Won't delete the entry but block the number of the chat.)
		$sql = 'UPDATE ajax_chat_channels ' .
		       'SET died = NOW() ' .
		       'WHERE manual_channel = 1 AND ' .
		       'died IS NULL AND ' .
		       'channel_id NOT IN (SELECT channel FROM `ajax_chat_online` GROUP BY channel);';
/*
		$sql = 'DELETE FROM ajax_chat_channels ' .
		       'WHERE manual_channel = 1 AND ' .
		       'channel_id NOT IN (SELECT channel FROM `ajax_chat_online` GROUP BY channel);';
*/
		// Create a new SQL query:
		...
		// Reset the local channel-cache if necessary
		if($result->affectedRows() > 0) {
			$this->getAllChannels(true);
			$this->getChannels(true); 
		}

		// Turn Zombies into dust (delete from database) 2 hours after the got deleted
		$sql = 'DELETE FROM ajax_chat_channels ' .
		       'WHERE died IS NOT NULL AND  ' .
		       'DATE_ADD(died, INTERVAL 3 HOUR) < \'' . date('Y-m-d H:i:s') . '\';';
        ...
        {
            $query = sprintf(
                "UPDATE ajax_chat_channels SET invite_only = 1 WHERE channel_id = ? AND created_by ?",
                $this->getChannel(),
                $this->getUserID()
            );
        }
        ...
        {
            $query = sprintf(
                "UPDATE ajax_chat_channels SET invite_only = 0 WHERE channel_id = ? AND created_by ?",
                $this->getChannel(),
                $this->getUserID()
            );
        }
