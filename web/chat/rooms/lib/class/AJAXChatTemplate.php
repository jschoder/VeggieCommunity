...
class AJAXChatTemplate {
	...
	function replaceTemplateTags($match) {
                $tag = $match[1];
                if (count($match) > 2) {
                    $tagContent = $match[2];
                } else {
		...
		switch($tag) {
			...
			case 'VC_LANG_CODE':
				if ($this->ajaxChat->getLangCode() === 'de') {
					return 'de';
				} else {
					return 'en';
				}
	...
	function getChannelTags() {
		...
		if($this->ajaxChat->isLoggedIn() && $this->ajaxChat->isAllowedToCreatePrivateChannel()) {
			$channels[$this->ajaxChat->getPrivateChannelName()] = $this->ajaxChat->getPrivateChannelID();
		}
		$channelOptions = '';
		$channelCount = 0;
		...
		{
			$channelCount++;
			$channelOptions .= sprintf('<div id="channel%d" class="%s"><a id="channelLink%d" class="inactivechannel" href="#" onclick="ajaxChat.switchChannel(\'%s\', %d);"><span id="channelName%d">%s</span> <span id="channelUserCount%d"></span></a></div>',
			                           $channelId,
			                           ($channelCount % 2 === 0 ? 'rowEven' : 'rowOdd'),
			                           $channelId,
			                           $this->ajaxChat->htmlEncode($channelName),
			                           $channelId,
			                           $channelId,
			                           $this->ajaxChat->htmlEncode($channelName),
			                           $channelId);
